...
public class HelloMultiLine extends FlexGanttFXSample {
	...
	protected GanttChart<?> createGanttChart() throws Exception {
		...
		graphics.setActivityRenderer(HelloActivity.class, GanttLayout.class,
				new HelloActivityRenderer(graphics, "Hello Activity Renderer"));
	...
	public Node getControlPanel() {
		...
		equalButton.setTooltip(new Tooltip(
				"Distribute available row height equally to all lines"));
		autoButton.setTooltip(new Tooltip(
				"Equal line height, activities non overlapping"));
		randomButton.setTooltip(new Tooltip(
				"Randomly place lines and allocate line height"));

		ToggleGroup toggleGroup = new ToggleGroup();
		toggleGroup.getToggles().addAll(equalButton, autoButton, randomButton);
		toggleGroup.selectedToggleProperty().addListener(
				it -> applyLineCount(row.getLineCount()));
		toggleGroup.selectedToggleProperty().addListener(
				it -> getGanttChart().getGraphics().showEarliestActivities());
	...
	private void applyLineCount(int count) {
		...
		for (int i = 0; i < 100; i++) {
			for (int j = 0; j < Math.random() * row.getLineCount() / 2; j++) {
				...
				Instant st = ZonedDateTime.of(date, time,
						ZoneId.systemDefault()).toInstant();
				Instant et = ZonedDateTime.of(date.plusDays(duration), time,
						ZoneId.systemDefault()).toInstant();
