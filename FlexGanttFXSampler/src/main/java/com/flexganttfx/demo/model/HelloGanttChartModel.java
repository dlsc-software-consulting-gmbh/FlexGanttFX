...
public class HelloGanttChartModel extends FlexGanttFXSample {
    ...
    public Node getControlPanel() {
        ...
        Button virtualFlowTest = new Button("VirtualFlow / TreeView Test");
        virtualFlowTest.setOnAction(event -> loadVirtualFlowTest());
        virtualFlowTest.setMaxWidth(Double.MAX_VALUE);

        box.getChildren().addAll(newModel, addLayer, showRoot, addSingleRow,
                addRows, setRows, removeRows, removeFirstTen, clearRows, loadTest, virtualFlowTest);
    ...
    private void loadVirtualFlowTest() {
        List<HelloRow> topLevelRows = new ArrayList<>();
        for (int i = 0; i < 30; i++) {
            HelloRow topLevelRow = new HelloRow("Top level row " + i);
            topLevelRows.add(topLevelRow);
            ...
            for (int j = 0; j < 20; j++) {
                HelloRow second = new HelloRow("second level: " + i + "/" + j);
                second.setHeight(30);
                topLevelRow.getChildren().add(second);

                for (int k = 0; k < 23; k++) {
                    HelloRow child = new HelloRow("child " + i + "/" + j + "/" + k);
                    child.setHeight((k * 2) + 30);
                    second.getChildren().add(child);

                    HelloActivity act = new HelloActivity();
                    act.setStartTime(ZonedDateTime.now().plusDays(3).plusHours(k * 3).toInstant());
                    act.setEndTime(act.getStartTime().plus(Duration.ofDays(k)));
                    child.addActivity(layer, act);
                }
            }
        }
