...
public class HelloLazyLoading extends FlexGanttFXSample {
	...
	protected GanttChart<?> createGanttChart() throws FileNotFoundException {
		...
		gantt.getTimeline()
				...
				.setOnVisibleRangeChanged(
						evt -> LazyLoadingManager.getInstance().ensureData(
								evt.getStartTime(), evt.getEndTime(), false));
	...
	static class LazyLoadingManager {
		...
		public void ensureData(Instant startTime, Instant endTime, boolean force) {
			if (!force
					&& (startTime.equals(lastStartTime) && endTime
							.equals(lastEndTime))) {
				return;
			}
		...
		private void afterSuccess(WorkerStateEvent evt) {
			...
			for (HelloLazyLoadingActivity activity : activities) {
				...
				MutableActivityRepositoryBase<HelloLazyLoadingActivity> repository = (MutableActivityRepositoryBase<HelloLazyLoadingActivity>) row
						.getRepository();
	 ...
	 */
	static class LazyLoadingService extends
			Service<List<HelloLazyLoadingActivity>> {
		...
		protected Task<List<HelloLazyLoadingActivity>> createTask() {
			return new LazyLoadingTask(startTime, endTime);
		}
		...
		protected List<HelloLazyLoadingActivity> call() throws Exception {
			...
			if (!isCancelled()) {
				...
				for (HelloLazyRow row : gantt.getGraphics().getRows()) {
					...
					Instant time = startTime.truncatedTo(ChronoUnit.DAYS);
					...
					if (!isCancelled()) {
						...
						while (time.isBefore(endTime)) {
							HelloLazyLoadingActivity activity = new HelloLazyLoadingActivity(
									row);
	...
	static class HelloLazyRow extends
			Row<HelloLazyRow, HelloLazyRow, HelloLazyLoadingActivity> {
