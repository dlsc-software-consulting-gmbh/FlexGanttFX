...
public class ChronoUnitDatelineCell extends DatelineCell<ChronoUnit> {
	...
	public void update(Instant startTime, Instant endTime,
			Resolution<ChronoUnit> resolution, Dateline dateline,
		...
		ZonedDateTime zonedStartTime = ChronoUnitUtils.truncate(
				ZonedDateTime.ofInstant(startTime, zoneId), temporalUnit, 1,
				dateline.getFirstDayOfWeek());

		switch (resolution.getTemporalUnit()) {
		case DAYS:
			getStyleClass().add(
					zonedStartTime.getDayOfWeek().toString().toLowerCase());
			break;
		case HALF_DAYS:
			getStyleClass().add(
					zonedStartTime.get(AMPM_OF_DAY) == 0 ? "am" : "pm");
			break;
		case MONTHS:
			getStyleClass().add(
					zonedStartTime.getMonth().toString().toLowerCase());
			break;
		default:
			break;
		}
