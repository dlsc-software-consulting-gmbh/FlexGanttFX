...
public class FlipPane<R extends Row<?, ?, ?>> extends StackPane {
	...
	private FlipPane(final Orientation FLIP_DIRECTION) {
		...
		backRotate = new Rotate(180,
				Orientation.HORIZONTAL == FLIP_DIRECTION ? Rotate.Y_AXIS
						: Rotate.X_AXIS);
	...
	public final void setRow(R row) {
		if (this.row != row) {
			...
			if (row != null) {
				GraphicsBase<R> graphics = rowCell.getGraphics();
				RowEditorParameter<R> param = new RowEditorParameter<>(
						graphics, row);
	...
	private void registerListeners() {
		...
		rotate.angleProperty().addListener(
				(ov, o, n) -> {
					if (Double.compare(o.doubleValue(), 90) < 0
							&& Double.compare(n.doubleValue(), 90) >= 0) {
	...
	private void hideControlsNode() {
		controlsNode = rowCell.getControlsNode();
	...
	public final void flipToFront() {
		...
		KeyValue kvRowHeight = new KeyValue(row.heightProperty(),
				previousRowHeight);

		KeyValue kvRotateStart = new KeyValue(rotate.angleProperty(), 180,
				Interpolator.EASE_IN);
		KeyValue kvRotateStop = new KeyValue(rotate.angleProperty(), 0,
				Interpolator.EASE_OUT);

		KeyValue kvScaleXStart = new KeyValue(scaleXProperty(), 1);
		KeyValue kvScaleYStart = new KeyValue(scaleYProperty(), 1);

		KeyValue kvScaleXMiddle = new KeyValue(scaleXProperty(), .8,
				Interpolator.EASE_BOTH);
		KeyValue kvScaleYMiddle = new KeyValue(scaleYProperty(), .8,
				Interpolator.EASE_BOTH);

		KeyValue kvScaleXStop = new KeyValue(scaleXProperty(), 1);
		KeyValue kvScaleYStop = new KeyValue(scaleYProperty(), 1);

		KeyFrame kfStart = new KeyFrame(Duration.ZERO, kvRotateStart,
				kvScaleXStart, kvScaleYStart);
		KeyFrame kfMiddle = new KeyFrame(Duration.millis(flipTime / 4),
				kvScaleXMiddle, kvScaleYMiddle);
		KeyFrame kfStop = new KeyFrame(Duration.millis(flipTime), kvRotateStop,
				kvScaleXStop, kvScaleYStop, kvRowHeight);
		...
		flipToFront.setOnFinished(event -> {
			...
			back.setCache(false);
			fireEvent(new FlipEvent(FlipPane.this, FlipPane.this,
					FlipEvent.FLIP_TO_FRONT_FINISHED));
	...
	public final void flipToBack() {
		...
		KeyValue kvRowHeight = new KeyValue(row.heightProperty(),
				calculatePrefHeightOfBack());

		KeyValue kvRotateStart = new KeyValue(rotate.angleProperty(), 0,
				Interpolator.EASE_IN);
		KeyValue kvRotateStop = new KeyValue(rotate.angleProperty(), 180,
				Interpolator.EASE_OUT);
		...
		flipToBack.setOnFinished(event -> {
			...
			back.setCache(false);
			fireEvent(new FlipEvent(FlipPane.this, FlipPane.this,
					FlipEvent.FLIP_TO_BACK_FINISHED));
			getScene().setCamera(null);
		});
		flipToBack.play();
	}
	...
	private double calculatePrefHeightOfBack() {
		return getBack().prefHeight(rowCell.getWidth());
	}
	...
	private void adjustRotationAxis() {
		if (front.getWidth() < 0 || back.getWidth() < 0
				|| front.getHeight() < 0 || back.getHeight() < 0) {
			return;
		}

		double width = front.getWidth() > back.getWidth() ? front.getWidth()
				: back.getWidth();
		double height = front.getHeight() > back.getHeight() ? front
				.getHeight() : back.getHeight();
	...
	public static class FlipEvent extends Event {
		...
		public static final EventType<FlipEvent> FLIP_TO_FRONT_FINISHED = new EventType<>(
				ANY, "flipToFrontFinished");

		public static final EventType<FlipEvent> FLIP_TO_BACK_FINISHED = new EventType<>(
				ANY, "flipToBackFinished");

		public FlipEvent(final Object SOURCE, final EventTarget TARGET,
				final EventType<FlipEvent> EVENT_TYPE) {
