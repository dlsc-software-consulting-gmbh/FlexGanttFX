...
public class DatelineSkin extends SkinBase<Dateline> {
	...
	private void buildCells() {
		...
		if (getDateline().getWidth() > 0 && getDateline().getHeight() > 0) {
			...
			getDateline().getProperties().put("com.flexganttfx.primaryUnit",
	...
	private void hideZoneIdLabel() {
		...
		ParallelTransition parallelTransition = new ParallelTransition(tt,
				fadeOut);
		parallelTransition.play();
	}
	...
	private void updateZoneIdText() {
		String text = getDateline().getZoneId().getDisplayName(FULL,
				Locale.getDefault());
