...
public class ColumnBrowser<S> extends Control {
	...
	public ColumnBrowser() {
		getStylesheets().add(
				ColumnBrowser.class.getResource("column-browser-view.css")
				.toExternalForm());

		tableView.addListener(evt -> getTableView()
				.setItems(getFilteredItems()));

		getColumnValuesLists().addListener(
				new ListChangeListener<ColumnValuesList<S, ?>>() {
					@Override
					public void onChanged(
							Change<? extends ColumnValuesList<S, ?>> c) {
						while (c.next()) {
							for (ColumnValuesList<S, ?> list : c
									.getAddedSubList()) {
								list.getListView().getSelectionModel()
	...
	private final ObservableList<ColumnValuesList<S, ?>> columnValuesLists = FXCollections
			.observableArrayList();
