This demo focuses on the built-in drag-and-drop support in **FlexGanttFX**. It shows how data can be dragged from external controls into a chart, moved within a chart, and exported again, while exposing the feedback, snapping, and event hooks involved in the interaction.
...
public class DragAndDropDemo extends DemoBase {
    ...
    public Node getControlPanel() {
        ...
        box.getChildren().add(new LabeledBox("Upper Gantt", new DragSettings(gantt1.getGraphics())));
        box.getChildren().add(new LabeledBox("Lower Gantt", new DragSettings(gantt2.getGraphics())));
        box.getChildren().add(new LabeledBox("Drag Info", dragInfoPane));

        return box;
    }

    static class LabeledBox extends HBox {

        public LabeledBox(String text, Node dragSettings) {
            Label label = new Label(text);
            setSpacing(10);
            getChildren().add(label);
            getChildren().add(dragSettings);
            HBox.setHgrow(dragSettings, javafx.scene.layout.Priority.ALWAYS);
            setAlignment(Pos.CENTER_LEFT);
        }
    }

    static class DragSettings extends HBox {
        ...
        public DragSettings(GraphicsBase<?> graphics) {
            ...
            Bindings.bindBidirectional(feedbackBox.valueProperty(), graphics.dragAndDropFeedbackProperty());
            getChildren().add(feedbackBox);

            CheckBox autogrid = new CheckBox("Gantt 1: Autogrid");

            Bindings.bindBidirectional(autogrid.selectedProperty(), graphics.autoGridEnabledProperty());
            ...
            ObservableList<VirtualGrid<?>> gridList = FXCollections.observableArrayList(days, hours, minutes);
    ...
    private void setupOrderTable() {
        ...
        orderTable.setOnDragDetected(this::dragDetectedOrderTable);
        orderTable.setOnDragDone(this::dragDoneOrderTable);
    }
    ...
    private void setupPersonTable() {
        ...
        personTable.setOnDragDetected(this::dragDetectedPersonTable);
    }
    ...
    private void dragDetectedPersonTable(MouseEvent evt) {
        ...
        ClipboardContent content = new ClipboardContent();
        content.putString("person:" + personTable.getSelectionModel().getSelectedItem().getName());
        db.setContent(content);
    }
    ...
    private void setupGantt() {
        ...
        graphics1.setOnDragOver(this::dragOver);
        graphics1.setOnDragDropped(this::dragDropped);
        ...
        graphics2.setOnDragOver(this::dragOver);
        graphics2.setOnDragDropped(this::dragDropped);
    ...
    private void setupTextDropArea() {
        ...
        textDropArea.setOnDragOver(this::dragOverOnTextArea);
        textDropArea.setOnDragDropped(this::dragDroppedOnTextArea);
    }
    ...
    private void dragDroppedOnTextArea(DragEvent evt) {
        ...
        textDropArea.appendText(System.lineSeparator());
        textDropArea.appendText(System.lineSeparator());
    ...
    private void setupTextEventsArea() {
        ...
        gantt1.getGraphics().setOnActivityChange(evt -> {
            ...
            textEventsArea.appendText(System.lineSeparator());
        });
    ...
    public Node getPanel(Stage stage) {
        ...
        verticalSplit.getItems().addAll(horizontalSplit, lowerSplit, textEventsArea);
    ...
    public static class OrderAssignment extends MutableActivityBase<Order> {
        ...
        public OrderAssignment(Order order) {
            ...
            setStartTime(ZonedDateTime.of(order.getStartDate(), LocalTime.MIN, ZoneId.systemDefault()).toInstant());
            setEndTime(ZonedDateTime.of(order.getEndDate(), LocalTime.MAX, ZoneId.systemDefault()).toInstant());
    ...
    static class OrderAssignmentRenderer extends ActivityRenderer<OrderAssignment> {
    ...
    public static class Resource extends Row<Resource, Resource, OrderAssignment> {
    ...
    static class DragInfoPane extends HBox {
        ...
        public DragInfoPane() {
            ...
            getChildren().setAll(activityLabel, activityField, rowLabel, rowField, transferModeLabel, transferModeField, startTimeLabel, startTimeField, endTimeLabel, endTimeField);
        ...
        public void setInfo(DragAndDropInfo info) {
            if (info != null) {
                activityField.setText(info.getActivityBounds().getActivity().getName());
                if (info.getRow() != null) {
                ...
                if (transferMode != null) {
                    transferModeField.setText(transferMode.toString());
                } else {
                    transferModeField.setText("NONE");
                }
                startTimeField.setValue(ZonedDateTime.ofInstant(info.getDropInterval().getStartTime(), ZoneId.systemDefault()).toLocalDate());
                endTimeField.setValue(ZonedDateTime.ofInstant(info.getDropInterval().getEndTime(), ZoneId.systemDefault()).toLocalDate());
            } else {
