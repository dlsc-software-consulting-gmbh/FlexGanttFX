This demo focuses on rendering a large number of dependency links. It is useful for exploring link performance and visual density when many relationships have to be drawn across the same chart.

```java
public class LinksStressTestDemo extends GanttChartDemoBase {

    private final ArrayList<ActivityLink<?>> links = new ArrayList<>();

    public LinksStressTestDemo() {
    }
