 ...
 */
public class HelloAtlantaFXStyling extends FlexGanttFXSample {
    ...
    protected GanttChartBase<?> createGanttChart() {
        ...
        gantt.getLayers().add(HelloRow.layer);

        HelloRow root = new HelloRow("root");
        root.setExpanded(true);
        for (int i = 0; i < 20; i++) {
            HelloRow task = new HelloRow("Task " + (i + 1), 3);
            if (i == 0 || i == 4 || i == 9 || i == 14) {
                int subtaskCount = (i == 4) ? 3 : 2;
                for (int j = 1; j <= subtaskCount; j++) {
                    task.getChildren().add(new HelloRow("Subtask " + (i + 1) + "." + j, 2));
                }
        ...
        TreeTableColumn<HelloRow, String> startCol = new TreeTableColumn<>("Start");
        ...
        TreeTableColumn<HelloRow, String> endCol = new TreeTableColumn<>("End");
